...
func (term *Term) Render() bool {
	...
	if term._scrollOffset != 0 {
		term.renderer.DrawGaugeV(
			term.tile,
			&types.XY{X: term.size.X, Y: 1}, term.size.Y-1,
			len(term._scrollBuf)-term._scrollOffset, len(term._scrollBuf),
			types.SGR_COLOR_BLACK_BRIGHT)
	}
