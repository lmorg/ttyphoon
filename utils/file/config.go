...
func GetConfigGlob(glob string) []string {
	...
	for _, dir := range xdg.ConfigDirs {
		configPath := fmt.Sprintf("%s/%s/%s", dir, app.DirName, glob)
...
func GetConfigFiles(subDir string, fileExt string) []string {
	...
	for _, dir := range xdg.ConfigDirs {
		configPath := fmt.Sprintf("%s/%s/%s/*%s", dir, app.DirName, subDir, fileExt)
		debug.Log(configPath)
...
func GetConfigFile(subDir string, resourceName string) (string, error) {
	for _, dir := range xdg.ConfigDirs {
		filename := fmt.Sprintf("%s/%s/%s/%s", dir, app.DirName, subDir, resourceName)
		debug.Log(filename)
